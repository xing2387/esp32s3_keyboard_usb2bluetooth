...
/**
 * Note:
 * 1. Win10 does not support vendor report , So SUPPORT_REPORT_VENDOR is always set to FALSE, it
 * defines in hidd_le_prf_int.h
 * 2. Update connection parameters are not allowed during iPhone HID encryption, slave turns
 * off the ability to automatically update connection parameters during encryption.
 * 3. After our HID device is connected, the iPhones write 1 to the Report Characteristic
 * Configuration Descriptor, even if the HID encryption is not completed. This should actually be
 * written 1 after the HID encryption is completed. we modify the permissions of the Report
 * Characteristic Configuration Descriptor to `ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE_ENCRYPTED`.
 * if you got `GATT_INSUF_ENCRYPTION` error, please ignore.
 */

#define HID_DEMO_TAG "HID_DEMO"

static int16_t hid_conn_id = -1;
...
void send_key_event(key_mask_t special_key_mask, uint8_t *keys, uint8_t num_key) {
    if (hid_conn_id >= 0) {
        esp_hidd_send_keyboard_value(hid_conn_id, special_key_mask, keys, num_key);
    } else {
        ESP_LOGE(HID_DEMO_TAG, "HID device not connected, hid_conn_id: %d", hid_conn_id);
    }
}
...
void ble_hid_init(void) {
    ...
    master should distribute to you, and the response key means which key you can distribute to the
    Master; If your BLE device act as a master, the response key means you hope which types of key
    of the slave should distribute to you, and the init key means which key you can distribute to
    the slave. */
