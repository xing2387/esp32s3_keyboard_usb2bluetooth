 ...
 * APP_EVENT_HID_HOST   - HID Host Driver event, such as device connection/disconnection or input
 * report.
 */
...
static void hid_host_keyboard_report_callback(const uint8_t *const data, const int length) {
    ...
    on_usb_keyboard_event(kb_report);
...
static void hid_host_mouse_report_callback(const uint8_t *const data, const int length) {
    ...
    on_usb_mouse_event(mouse_report);
...
void hid_host_interface_callback(hid_host_device_handle_t hid_device_handle,
    ...
    switch (event) {
        case HID_HOST_INTERFACE_EVENT_INPUT_REPORT:
            ESP_ERROR_CHECK(hid_host_device_get_raw_input_report_data(hid_device_handle, data, 64,
                                                                      &data_length));
...
static void hid_host_device_event_task(void *arg) {
        ...
        if (xQueueReceive(app_event_queue, &evt_queue, portMAX_DELAY)) {
            if (APP_EVENT_HID_HOST == evt_queue.event_group) {
                ...
                if (lib_info.num_devices == 0) {
                    ...
                    ESP_LOGW(TAG,
                             "To shutdown example, remove all USB devices and press button again.");
                    // Keep polling
                }
            }
        }
...
void usb_host_init(void) {
    ...
    //             } else {
    //                 ESP_LOGW(TAG,
    //                          "To shutdown example, remove all USB devices and press button
    //                          again.");
    //                 // Keep polling
    //             }
    //         }
    ...
    xTaskCreate(&hid_host_device_event_task, "hid_device_event_task", 4096, NULL, 5, NULL);
    while (1) {
        vTaskDelay(10000);
    }
}
